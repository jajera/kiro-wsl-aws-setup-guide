...
Kiro has a CLI similar to VS Code: the `kiro` command (not `kiro-cli`, which is a separate terminal AI tool). For custom agents, MCP integration, and hooks in the terminal, see the [Kiro CLI](../kiro-cli/custom-agents) section.
